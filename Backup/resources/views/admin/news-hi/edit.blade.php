                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <form id="editform" method="post" class="form-horizontal" action="{{url('admin/news/hindi/update')}}">
                                {{csrf_field()}}
                                {{ method_field('POST') }}
                                <div class="row">
                                    <div class="col-sm-12">
                                        <div class="card-body">
                                                ...
                                                <div class="col-sm-8">
                                                    <div>
                                                        <select name="tags" id="tagId" class="form-control customError" required>
                                                            <option value="{{$singleNews->primary_tag_id}}" selected>{{$singleNews->getTagName->name}}</option>
                                                        </select>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8">
                                                    <div style="margin-bottom: 10px" class="news-image {{empty($singleNews->image_path) ?'dnone':''}}" id="image-preview">
                                                        @if(strstr($singleNews->image_path,"/"))

                                                            <img class="preview" src="  {{ env('S3_BUCKET_URL2','').trim($singleNews->image_path,'/') }}" />
                                                        @else
                                                             <img class="preview" src="{{env('S3_BUCKET_URL','').Config('constants.NEWS_HINDI_UPLOAD_PATH').trim($singleNews->image_path,'/')}}" />
                                                        @endif
                                                            <span class="news-delete icon-delete" data-id="{{$singleNews->id}}"><i class="fas fa-trash"></i></span>
                                                    </div>
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8">
                                                    <input type="hidden" name="id" id="newsId" value="{{$singleNews->id}}">
                                                    <input type="text" name="title" class="form-control" value="{{$singleNews->title}}"  placeholder="Title"  required>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label for="inputPassword3" class="col-sm-3 col-form-label">Home Title</label>
                                                <div class="col-sm-8">
                                                    <input type="text" name="home_title" class="form-control" value="{{$singleNews->home_title}}" placeholder="Home Title"  required>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label for="inputPassword3" class="col-sm-3 col-form-label">Short Description</label>
                                                <div class="col-sm-8">
                                                    <input type="text" name="short_description" class="form-control"  value="{{$singleNews->short_desc}}"  placeholder="Short Description"  required>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8">
                                                    <div>
                                                        <select class="form-control customError" name="author" id="authorId" required>
                                                            <option value="{{$singleNews->author_id}}" selected>{{$singleNews->getAuthorName->name}}</option>
                                                        </select>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div class="col-sm-8">
                                                    <div>
                                                        <select class="form-control customError" name="associated_tags[]" id="associatedTags" multiple required>
                                                            @foreach($singleNews->getAssocTags as $tags)
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8">
                                                    <select id="inputStatus" name="status" class="form-control custom-select" required>
                                                        ...
                                                        <option value="1" {{$singleNews->status == 1 ? 'selected' : ''}}>Active</option>
                                                        <option value="0" {{$singleNews->status == 0 ? 'selected' : ''}}>Inactive</option>
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-11">
                                                    <textarea name="description" id="inputDescription" class="form-control" minlength="2" required>{{ $singleNews->content }}</textarea>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <div class="col-sm-12 text-center">
                                                    <button type="submit" class="btn btn-large btn-success btn-lg">Update News</button>
                                                </div>
    ...
    <script>
        $( document ).ready(function() {
            ...
            $('#authorId').select2({
                ...
                ajax: {
                    url: '{{url("admin/news/hindi/get-authors")}}',
                    dataType: 'json',
                    delay: 250,
            ...
            $('#associatedTags').select2({
                ...
                ajax: {
                    url: '{{url("admin/news/hindi/get-kickers")}}',
                    dataType: 'json',
                    delay: 250,
                    processResults: function (data) {
                        return {
        ...
        async function upload(formdata){
            return $.ajax({
                url: baseUrl+"/admin/news/hindi/image/upload",
                type: "POST",
                data: formdata,
                processData: false,
                contentType: false
